...
def get_test_files(cfg):
    ...
    def visit(ignored, dir, files):
        # amended by rgrp wish to allow test files anywhere ....
...
#            return
        for file in files:
            # amended by rgrp to include *test.py or *Test.py
            if (file.startswith('test') and file.endswith('.py')) or file.endswith('test.py') or file.endswith('Test.py'):
...
def get_all_test_cases(module):
    ...
    for name in dir(module):
        # amended by rgrp: allow *Test as well as Test*
        if not (name.startswith('Test') or name.endswith('Test')):
            continue
